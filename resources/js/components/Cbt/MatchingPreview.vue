...
const pairs = computed(() => {
    ...
    if (raw[0] && typeof raw[0] === 'object' && !Array.isArray(raw[0]) && 'kiri' in raw[0]) {
        return raw;
    }
...
const stableShuffle = (arr: Array<any>) => {
    ...
    return newArr.sort((a, b) => {
        const scoreA = a.text.split('').reduce((acc: number, char: string) => acc + char.charCodeAt(0), 0);
        const scoreB = b.text.split('').reduce((acc: number, char: string) => acc + char.charCodeAt(0), 0);
        return scoreA - scoreB;
    });
};

watch(() => pairs.value, (newPairs) => {
...
const updateLines = () => {
    ...
    pairs.value.forEach((pair, leftIdx) => {
        const leftEl = containerRef.value?.querySelector(`.left-dot-${leftIdx}`);
        // Right dot has class right-dot-{leftIdx} because its id is leftIdx
        const rightEl = containerRef.value?.querySelector(`.right-dot-${leftIdx}`);
...
onMounted(() => {
    ...
    if (window.MutationObserver && containerRef.value) {
        ...
        observer.observe(containerRef.value, { childList: true, subtree: true, attributes: true });
...
<template>
    <div ref="containerRef" class="relative grid grid-cols-12 gap-8 my-4 p-4 border border-gray-100 rounded-xl bg-gray-50/50">
        ...
        <svg class="absolute inset-0 w-full h-full pointer-events-none z-20">
            ...
            <path 
                ...
                class="opacity-80 transition-all duration-300 hover:opacity-100 hover:stroke-[3.5px]"
            />
        </svg>

        <!-- Left Column: Statements -->
        <div class="col-span-5 space-y-4 z-10">
            <div class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Pernyataan (Kiri)</div>
            <div 
                v-for="(item, idx) in pairs" 
                :key="'left-'+idx" 
                class="flex items-center justify-between p-4 bg-white border border-gray-200 rounded-lg shadow-sm relative min-h-[50px] transition-all hover:shadow-md"
            >
                <div class="flex items-center gap-3">
                    ...
                    <div class="text-sm font-medium text-gray-700 leading-relaxed prose max-w-none" v-html="item.kiri"></div>
                </div>
                <!-- Connector Dot -->
                <div 
                    :class="['left-dot-' + idx]"
                    class="w-3.5 h-3.5 rounded-full bg-blue-500 border-2 border-white shadow-md absolute -right-[7px] top-1/2 -translate-y-1/2 cursor-default"
                ></div>
            </div>
        </div>
        ...
        <div class="col-span-5 space-y-4 z-10">
            <div class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Jawaban Pasangan (Kanan)</div>
            <div 
                v-for="(item, idx) in rightList" 
                :key="'right-'+item.id" 
                class="flex items-center p-4 bg-white border border-gray-200 rounded-lg shadow-sm relative min-h-[50px] transition-all hover:shadow-md"
            >
                <!-- Connector Dot -->
                <div 
                    :class="['right-dot-' + item.id]"
                    class="w-3.5 h-3.5 rounded-full bg-emerald-500 border-2 border-white shadow-md absolute -left-[7px] top-1/2 -translate-y-1/2 cursor-default"
                ></div>
