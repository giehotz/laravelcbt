...
import { store as storeSesi, update as updateSesi, destroy as destroySesi } from '@/routes/cbt/sesi';
...
<template>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y divide-neutral-200 dark:divide-zinc-800 text-sm">
                        <tr v-for="(item, idx) in sesi.data" :key="item.id" class="hover:bg-neutral-50/30 dark:hover:bg-zinc-800/20 transition-colors">
                            <td class="px-6 py-3.5 text-center font-medium text-neutral-400 dark:text-neutral-500">{{ idx + 1 }}</td>
                            <td class="px-6 py-3.5 font-bold text-neutral-800 dark:text-neutral-200">{{ item.nama_sesi }}</td>
                            <td class="px-6 py-3.5 font-mono text-xs font-semibold text-neutral-500 dark:text-neutral-405">{{ item.kode_sesi }}</td>
                            <td class="px-6 py-3.5 text-center font-semibold text-neutral-700 dark:text-neutral-300">{{ item.waktu_mulai }}</td>
                            <td class="px-6 py-3.5 text-center font-semibold text-neutral-700 dark:text-neutral-300">{{ item.waktu_akhir }}</td>
                            <td class="px-6 py-3.5 text-center">
                                <button
                                    type="button"
                                    @click="toggleAktif(item)"
                                    class="inline-flex items-center justify-center rounded-full px-2.5 py-0.5 text-xs font-semibold transition-colors focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2"
                                    :class="item.aktif ? 'bg-emerald-100 text-emerald-800 hover:bg-emerald-200 dark:bg-emerald-900/30 dark:text-emerald-400' : 'bg-neutral-100 text-neutral-800 hover:bg-neutral-200 dark:bg-neutral-800 dark:text-neutral-400'"
                                >
    ...
    <div v-if="isAddModalOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm transition-opacity">
        <div class="bg-white dark:bg-zinc-900 border border-neutral-200 dark:border-zinc-800 w-full max-w-md rounded-xl shadow-xl overflow-hidden flex flex-col">
            <div class="px-6 py-4 border-b border-neutral-200 dark:border-zinc-800 flex justify-between items-center">
                <h3 class="font-bold text-neutral-800 dark:text-neutral-200">Tambah Sesi Ujian</h3>
            ...
            <form @submit.prevent="submitAdd" class="p-6 space-y-4">
                <div class="grid gap-2">
                    <Label for="nama_sesi">Nama Sesi <span class="text-rose-500">*</span></Label>
                    <Input id="nama_sesi" v-model="addForm.nama_sesi" required placeholder="Contoh: Sesi 1" />
                    <InputError :message="addForm.errors.nama_sesi" />
                </div>
                <div class="grid gap-2">
                    <Label for="kode_sesi">Kode Sesi <span class="text-rose-500">*</span></Label>
                    <Input id="kode_sesi" v-model="addForm.kode_sesi" required placeholder="Contoh: S1" />
                    <InputError :message="addForm.errors.kode_sesi" />
                </div>
                <div class="grid grid-cols-2 gap-4">
                    <div class="grid gap-2">
                        <Label for="waktu_mulai">Jam Mulai <span class="text-rose-500">*</span></Label>
                        <Input id="waktu_mulai" type="time" v-model="addForm.waktu_mulai" required />
                        <InputError :message="addForm.errors.waktu_mulai" />
                    </div>
                    <div class="grid gap-2">
                        <Label for="waktu_akhir">Jam Akhir <span class="text-rose-500">*</span></Label>
                        <Input id="waktu_akhir" type="time" v-model="addForm.waktu_akhir" required />
                        <InputError :message="addForm.errors.waktu_akhir" />
                    </div>
                </div>
                <div class="flex items-center gap-2 pt-2">
                    <input type="checkbox" id="aktif" v-model="addForm.aktif" class="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-600" />
                    <Label for="aktif" class="cursor-pointer">Status Aktif</Label>
                </div>
    ...
    <div v-if="isEditModalOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm transition-opacity">
        <div class="bg-white dark:bg-zinc-900 border border-neutral-200 dark:border-zinc-800 w-full max-w-md rounded-xl shadow-xl overflow-hidden flex flex-col">
            <div class="px-6 py-4 border-b border-neutral-200 dark:border-zinc-800 flex justify-between items-center">
                <h3 class="font-bold text-neutral-800 dark:text-neutral-200">Edit Sesi Ujian</h3>
            ...
            <form @submit.prevent="submitEdit" class="p-6 space-y-4">
                <div class="grid gap-2">
                    <Label for="edit_nama_sesi">Nama Sesi <span class="text-rose-500">*</span></Label>
                    <Input id="edit_nama_sesi" v-model="editForm.nama_sesi" required placeholder="Contoh: Sesi 1" />
                    <InputError :message="editForm.errors.nama_sesi" />
                </div>
                <div class="grid gap-2">
                    <Label for="edit_kode_sesi">Kode Sesi <span class="text-rose-500">*</span></Label>
                    <Input id="edit_kode_sesi" v-model="editForm.kode_sesi" required placeholder="Contoh: S1" />
                    <InputError :message="editForm.errors.kode_sesi" />
                </div>
                <div class="grid grid-cols-2 gap-4">
                    <div class="grid gap-2">
                        <Label for="edit_waktu_mulai">Jam Mulai <span class="text-rose-500">*</span></Label>
                        <Input id="edit_waktu_mulai" type="time" v-model="editForm.waktu_mulai" required />
                        <InputError :message="editForm.errors.waktu_mulai" />
                    </div>
                    <div class="grid gap-2">
                        <Label for="edit_waktu_akhir">Jam Akhir <span class="text-rose-500">*</span></Label>
                        <Input id="edit_waktu_akhir" type="time" v-model="editForm.waktu_akhir" required />
                        <InputError :message="editForm.errors.waktu_akhir" />
                    </div>
                </div>
                <div class="flex items-center gap-2 pt-2">
                    <input type="checkbox" id="edit_aktif" v-model="editForm.aktif" class="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-600" />
                    <Label for="edit_aktif" class="cursor-pointer">Status Aktif</Label>
                </div>
