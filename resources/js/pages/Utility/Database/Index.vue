...
const executeTruncate = () => {
    if (truncateForm.module !== truncateForm.confirmation) {
        truncateForm.setError(
            'confirmation',
            'Konfirmasi nama modul tidak cocok!',
        );
        return;
    }
        ...
        <div class="mx-auto max-w-7xl py-6 sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                ...
                <div class="overflow-hidden bg-white shadow sm:rounded-lg">
                    <div
                        class="flex items-center border-b border-gray-200 bg-gray-50 px-4 py-5 sm:px-6"
                    >
                        <svg
                    ...
                    <div class="p-6">
                        <p class="mb-6 text-sm text-gray-600">
                            Fitur ini akan mengekspor seluruh database aplikasi
                            ke dalam file
                            <span class="rounded bg-gray-100 px-1 font-mono"
                                >.sql</span
                            >
                            dan langsung mengunduhnya ke perangkat Anda.
                            Pastikan Anda melakukan backup secara rutin sebelum
                            melakukan perubahan besar atau pembersihan data.
                        </p>
                        <form
                            method="POST"
                            :action="route('utility.database.backup')"
                        >
                            <!-- CSRF token needed for normal form post if we don't use Inertia or window.location -->
                            <button
                                type="button"
                                @click="executeBackup"
                                class="inline-flex items-center justify-center rounded-md border border-transparent bg-indigo-600 px-4 py-2 font-medium text-white shadow-sm hover:bg-indigo-700 focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 focus:outline-none"
                            >
                                <svg
                                    class="mr-2 h-5 w-5"
                                    xmlns="http://www.w3.org/2000/svg"
                ...
                <!-- Data Manager Card -->
                <div
                    class="overflow-hidden border-t-4 border-red-500 bg-white shadow sm:rounded-lg"
                >
                    <div
                        class="flex items-center border-b border-gray-200 bg-gray-50 px-4 py-5 sm:px-6"
                    >
                    ...
                    <div class="p-6">
                        <div
                            class="mb-6 border-l-4 border-red-400 bg-red-50 p-4"
                        >
                            <div class="flex">
                                <div class="ml-3">
                                    <p
                                    ...
                                    >
                                        Perhatian: Aksi ini bersifat permanen
                                        (TIDAK BISA DI-UNDO).
                                    </p>
                                    <p class="mt-1 text-sm text-red-700">
                                        Fungsi ini digunakan untuk mengosongkan
                                        tabel transaksional (seperti nilai, log
                                        ujian) guna persiapan semester baru.
                                        Tabel master (siswa, guru, soal)
                                        dilindungi dari penghapusan massal.
                                    </p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="space-y-4">
                            ...
                            >
                                ...
                                <button
                                    @click="openTruncateModal(mod)"
                                    class="rounded bg-red-100 px-3 py-1 text-sm font-medium text-red-600 hover:text-red-900"
                                >
                                    Kosongkan Data
                                </button>
                            </div>
                ...
                >
                                ...
                                <div class="mt-2">
                                    <p class="text-sm text-gray-500">
                                        ...
                                        <strong class="text-red-600"
                                            >MENGHAPUS SEMUA DATA</strong
                                        >
                                        pada modul ini secara permanen. Anda
                                        tidak dapat mengembalikan data ini
                                        kembali.
                                    </p>
                                    <div class="mt-4">
                                        <label
                                        ...
                                        >
                                            Untuk melanjutkan, ketik ulang nama
                                            modul "<span
                                                class="font-mono font-bold"
                                                >{{ truncateForm.module }}</span
                                            >":
                                        </label>
                                        <input
                                            v-model="truncateForm.confirmation"
                                            type="text"
                                            class="mt-1 block w-full rounded-md border-gray-300 font-mono shadow-sm focus:border-red-500 focus:ring-red-500 sm:text-sm"
                                            :placeholder="truncateForm.module"
                                        />
                                        <p
                                            v-if="
                                                truncateForm.errors.confirmation
                                            "
                    ...
                    >
                        <button
                            ...
                            :disabled="
                                truncateForm.processing ||
                                truncateForm.module !==
                                    truncateForm.confirmation
                            "
                            class="inline-flex w-full justify-center rounded-md border border-transparent px-4 py-2 text-base font-medium text-white shadow-sm sm:ml-3 sm:w-auto sm:text-sm"
                            :class="
                                truncateForm.module ===
                                truncateForm.confirmation
                                    ? 'bg-red-600 hover:bg-red-700 focus:ring-2 focus:ring-red-500 focus:ring-offset-2 focus:outline-none'
