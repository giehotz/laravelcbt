...
class CbtService
{
    ...
    public function distribusiSoal(Siswa $siswa, Jadwal $jadwal): void
    {
        DB::transaction(function () use ($siswa, $jadwal) {
            ...
            $sudahDistribusi = CbtSoalSiswa::where('siswa_id', $siswa->id)
                ->where('jadwal_id', $jadwal->id)
                ->exists();
            ...
            foreach ($groupedSoals as $jenis => $soalsPerJenis) {
                ...
                foreach ($soalsPerJenis as $index => $soal) {
                    ...
                    if ($jadwal->acak_opsi && in_array($jenis, [1, 2])) {
                        ...
                        $availableOpsis = [];
                        if ($soal->opsi_a || $soal->fileA) $availableOpsis[] = 'A';
                        if ($soal->opsi_b || $soal->fileB) $availableOpsis[] = 'B';
                        if ($soal->opsi_c || $soal->fileC) $availableOpsis[] = 'C';
                        if ($soal->opsi_d || $soal->fileD) $availableOpsis[] = 'D';
                        if ($soal->opsi_e || $soal->fileE) $availableOpsis[] = 'E';
                    ...
                    $soalSiswaInsertData[] = [
                        ...
                        'jawaban_benar' => $soal->jawaban, // Cache jawaban benar ori (tetap tersembunyi)
            ...
            if (!empty($soalSiswaInsertData)) {
                ...
                CbtSoalSiswa::insert($soalSiswaInsertData);
            }
        });
    }
    ...
    {
        DB::transaction(function () use ($siswa, $jadwal) {
            $soalSiswas = CbtSoalSiswa::with('soal.pairs')
