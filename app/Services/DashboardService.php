...
{
    /**
     * Get aggregated dashboard statistics and data based on user role level.
     */
    public function getDashboardData(User $user, TahunPelajaran $tp, Semester $smt): array
    {
        $roleLevel = $user->getRoleLevel();

        $data = match (true) {
            $roleLevel >= 100 => $this->dataAdmin($tp, $smt),
            $roleLevel >= 80 => $this->dataKepsek($tp, $smt),
            $roleLevel >= 60 => $this->dataOperator($tp, $smt),
            $roleLevel >= 50 => $this->dataProktor($tp, $smt),
            $roleLevel >= 40 => $this->dataGuru($user, $tp, $smt),
            default => ['stats' => []],
        };

        $data['jadwalHariIni'] = $this->jadwalHariIni($tp, $smt, $user);
    ...
    private function dataGuru(User $user, TahunPelajaran $tp, Semester $smt): array
    {
        $guruId = $user->guru?->id;
        ...
        $stats = Cache::remember("dashboard_guru_{$guruId}_{$tp->id}_{$smt->id}", 300, fn () => [
            ...
            'total_soal_saya' => Soal::whereHas('bank', function ($q) use ($guruId) {
    ...
    private function jadwalHariIni(TahunPelajaran $tp, Semester $smt, User $user)
    {
        ...
        if ($user->getRoleLevel() === 40) { // exactly Guru
            $guruId = $user->guru?->id;
